...
# custom
from test_system.routes.personal_data_answer import ROUTE
from test_system.models import TestAnswer


@fixture()
def personal_data_answer_default_query_string(personal_data_test_name) -> Dict[str, str]:
    return {"test-name": personal_data_test_name}


def test_post_personal_data_answer__with_success(client: FlaskClient, session,
    ...
    for data_name, test_data in test_cases.items():
        resp = client.post(ROUTE, data=json_dumps(test_data), query_string=personal_data_answer_default_query_string)
        ...
        assert personal_data_answer is not None, (f"Could not write personal data answer to database"
                                                  f"\n\nReceived response:\n{resp.get_data(True)}")

        assert personal_data_answer.answer_json == test_data, ("Personal data written to database is wrong"
                                                               f"\n\nReceived response:\n{resp.get_data(True)}")
    ...
    with raise_if_change_in_tables(TestAnswer):
        for wrong_query_string in wrong_query_strings:
            ...
            assert resp.status_code == 400, (f"Got wrong status code at {ROUTE} for request with missing test name"
                                             f"\n\nReceived response:\n{resp.get_data(True)}")
        for data_name, test_data in test_cases.items():
            resp = client.post(ROUTE, data=test_data, query_string=personal_data_answer_default_query_string)
            assert resp.status_code == 400, (f"Got wrong status code at {ROUTE} for {data_name}: {test_data}"
                                             f"\n\nReceived response:\n{resp.get_data(True)}")


def test_post_personal_data_answer__with_unknown_test(client: FlaskClient, session, raise_if_change_in_tables):
