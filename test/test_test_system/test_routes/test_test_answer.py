...
def test_post_test_answer__with_bad_request(client, session, person, test):
    session.add_all((person, test))
    session.commit()

    test_arguments = [{"test-name": test.name}, {"person-id": person.id}]
...
def test_post_test_answer__with_wrong_test(client, session, person, personal_data_test):
...
def test_post_test_answer__with_wrong_person(client, session, test):
    session.add(test)
    session.commit()
