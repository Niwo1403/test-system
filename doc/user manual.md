...
- On the start screen at https://your-domain.com/index.html a token (&rarr; [about tokens in database](#token-table)) can be entered to execute a personality test, which consists of:
  ...
  - One _EVALUABLE_TEST_, which must only return numeric values and will be evaluated
...
- Contains the names (to identify the test), descriptions (which define the questions, etc.) and categories of the tests
  - The test category can be:
    ...
    - _EVALUABLE_TEST_: for tests which will be evaluated (must only collect numeric answers)
