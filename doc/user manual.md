...
- Contains the tokens, consisting of the actual token, information regarding usages & expiring, and the tests, which should be answered
  - Each token got exactly one _PERSONAL_DATA_TEST_ (to identify test answerer) and _EVALUABLE_TEST_ (which will be exported), but there can be any number of _PRE_COLLECT_TESTS_
