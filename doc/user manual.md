...
- After answering all questions, the user will be redirected to the result side, showing a PDF with the answers
  - The user is only able to generate the PDF, if the token isn't expired and has at least one usage left (a token can have multiple usages)
